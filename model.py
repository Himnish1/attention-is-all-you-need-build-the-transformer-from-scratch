...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    len_specials = len(specials_list)
    return_dict = {key: index for index, key in enumerate(specials_list)}
    # build corpus
    corpus = []
    for sentence in sentences:
        words = sentence.split(' ')
        for word in words:
            if word not in corpus and word not in specials:
                corpus.append(word)

    word_dict = {key: index+len_specials for index, key in enumerate(corpus)}
...
def build_id_to_token_vocab(token_to_id):
    ...
    return {value: key for key, value in token_to_id.items()}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in word_list:
        index = token_to_id.get(word, token_to_id[unk_token])
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    return_list = []
    for index in ids:
        return_list.append(id_to_token.get(index))
